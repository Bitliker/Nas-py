import os
from engine_tmdb import *
from entry import MediaType
from libs_nfo import *
from libs_file import *
from libs_subtitle import sync_subtitle
from libs_video import traverse_video, is_video, get_video_title
from libs_image import download_image, frame_video_image
...
class tv_client:
    ...
    def scraping_by_tmid(self, tmid, file_name_title, file_path, target_dir):
        ...
        return True
    """保存演员图片"""
    def save_actor(self , url , file_path):
        # 判断图片是否下载过
        if os.path.exists(file_path):
            return
        file_name = os.path.basename(file_path)
        save_actor_dir = os.getenv("save_actor_dir")
        if not save_actor_dir:
            return
        actor_file = os.path.join(save_actor_dir,file_name)
        if not os.path.exists(actor_file):
            download_image(url=url,file_path=actor_file)
        if os.path.exists(actor_file):
            dir = os.path.dirname(file_path)
            os.makedirs(dir,exist_ok=True)
            shutil.copy2(actor_file, file_path)
...
path = os.getenv("source_dir_tv")
if not path:
    print("请设置 source_dir_tv 环境变量")
    exit()
client = tv_client()
client.scraping_batch(path)